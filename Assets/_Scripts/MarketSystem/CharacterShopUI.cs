...
public class CharacterShopUI : MonoBehaviour
{
	...
	void GenerateShopItemsUI()
	{
		...
		for (int i = 0; i < characterDB.CharactersCount; i++)
		{
			...
			Blade character = characterDB.GetCharacter(i);
			...
			uiItem.SetCharacterPower(character.damage);
	...
	{
		Blade character = characterDB.GetCharacter(index);
		CharacterItemUI uiItem = GetItemUI(index);

		if (GameDataManager.CanSpendCoins(character.price,CollectType.gem))
		{
			//Proceed with the purchase operation
			GameDataManager.SpendCoins(character.price,CollectType.gem);
